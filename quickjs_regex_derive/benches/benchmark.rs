...
fn regex_test(c: &mut Criterion) {
    c.bench_function("regex_test", |b| b.iter(|| Regex::new(r"^bc(d|e)*$").unwrap().is_match(black_box(TEXT))));
}

fn quickjs_regex_test(c: &mut Criterion) {
    c.bench_function("quickjs_regex_test", |b| b.iter(|| uregex!("^bc(d|e)*$").test(black_box(TEXT))));
}
