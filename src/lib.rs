...
extern {
    fn lre_compile(byte_code_len:* mut isize,error_msg:*mut c_char,error_msg_size:isize,pattern:*const c_char , pattern_len: isize, flag : isize, ctx: *mut c_void)->*mut u8;
    fn lre_exec(capture:*mut*mut u8,bc_buf: *const u8, cbuf:*const u8, cindex: isize, clen : isize, cbuf_type : isize, opaque: *mut c_void)->isize;
...
impl Regex {
    pub fn complie(pattern:&str,flag:Flag)->Result<Regex,String>{
        ...
        let byte_code_ptr = unsafe {
            lre_compile(&mut byte_code_len as *mut isize,error_msg.as_mut_ptr(), ERROR_MSG_SIZE , cpattern.as_ptr() as *const c_char , pattern.len() as isize , flag.0 , std::ptr::null_mut() )
        };
        if byte_code_ptr == std::ptr::null_mut() {
            return Err(unsafe {CStr::from_ptr(error_msg.as_ptr())}.to_string_lossy().into_owned());
        }
        let byte_code_len = byte_code_len.try_into().map_err(|_|"byte_code_len error".to_string())?;
        let code = unsafe {Vec::from_raw_parts(byte_code_ptr, byte_code_len, byte_code_len)};
        let count = unsafe {
            lre_get_capture_count(code.as_ptr() as *const _) as usize
        };
        Ok(Regex{code,count})
    }
    pub fn exec<'a>(&self,s:&'a str)->Result<Option<Vec<&'a str>>,String>{
        ...
        let r = unsafe {
            lre_exec(cap.as_mut_ptr() as *mut *mut u8,self.code.as_ptr() as *const _ ,s_ptr as *const _,0,s.len() as isize,0,std::ptr::null_mut()as *mut _)
        };
        match r {
            1 => (),
            0 => return Ok(None),
        ...
        while let Some(start) = iter.next() {
            ...
            let slice = unsafe {std::str::from_utf8_unchecked(std::slice::from_raw_parts(*start as *mut u8,*end-*start))};
